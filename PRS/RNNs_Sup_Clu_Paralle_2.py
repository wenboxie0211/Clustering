"""
实验2，因为之前的稳定性很差，在这个改版中，我们尝试减枝的操作
实验3，尝试做一个确定根结点的方法
...
if __name__ == '__main__':
    ...
    file_name = '/Users/wenboxie/Data/uci-20070111/exp/waveform-5000.txt'
    ...
    for i in range(1):
        ...
        print('ri_RS(k=', len(set(r.values())), ') =', metrics.cluster.mutual_info_score(label, RS_labels_))
    ...
    print('ri_kmeans =', metrics.cluster.mutual_info_score(label, kmeans_model.labels_))

    birch_model = cluster_methods.Birch(n_clusters=3).fit(data)
    print('ri_birch =', metrics.cluster.mutual_info_score(label, birch_model.labels_))

    agg_model = cluster_methods.AgglomerativeClustering(n_clusters=3).fit(data)
    print('ri_GA =', metrics.cluster.mutual_info_score(label, agg_model.labels_))
